...
- **Spine 深度互动支持**：完整支持官方拖拽反应链（`drag1/2/3` → `ex` 循环 → `drag_ex` → `normal`）、多部位触摸轮换（摸头/身体/特殊触摸），以及纯表情皮肤点击切换表情
- **Live2D 皮肤**（`live2d`，Cubism 3/4 完整动作、物理与视线追踪）
- **静态立绘**（`painting` 碎片重组，基于 azur-paint 自动图层合成）
- 官方 CDN 握手抓取 + AssetBundle 本地解包提取（支持断点续传与差分更新）
- 图鉴式 Web 预览（Spine / Live2D 双引擎，自由平移取景，缩放范围支持 **20% ~ 800%**）
- **互动语音**：官方 CriWare cue 语音包抓取 + vgmstream 无窗高速解码，点击角色播放原声台词与字幕气泡
- **Wiki 图鉴自动同步**：自动增量同步 B 站 Wiki 角色与换装中文名，支持单页换装增量补录与新船别名映射，告别代码代号
- 导出 Wallpaper Engine 壁纸项目（含 project.json 原生属性、一键应用；右侧面板可实时调节 **20%~800% 缩放**，并随时切换**语音 / 开场动画 / 互动**开关）
